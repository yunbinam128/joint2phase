obs_loglik <- function(theta2, p_covs_m2, y2_vec, y1_vec, Xmat_m2, mu1, alpha_ext) {
  # Parameter mapping
  ...
  dens_y2 <- pmax(1e-16, stats::dnorm(y2_vec, mean = mu2, sd = sqrt(sigma22)))

  # Observed-data log-likelihood: log[P(Y1, Y2)] = log[P(Y1 | Y2) * P(Y2)]
  return(sum(log(prob_y1_given_y2) + log(dens_y2)))
}

score_theta2_function <- function(theta2, p_covs_m2, y2_vec, y1_vec, Xmat_m2, mu1, alpha_ext) {
  # Parameter mapping
  gamma   <- theta2[1:p_covs_m2]
  sigma12 <- theta2[p_covs_m2 + 1]
  sigma22 <- theta2[p_covs_m2 + 2]
...
weighted_alpha_nll <- function(alpha, beta, y, X, weights) {
  ...
  if (!is.finite(log_lik)) return(1e10)  # Prevent non-finite returns to optim
...
calculate_full_loglik_theta12 <- function(theta1, theta2, p_vl, y1_s0, y1_s1, y2_s1,
                                          Xmat_m1_s0, Xmat_m1_s1, Xmat_m2, B0, x_support, x_name) {

  # Subjects selected in Phase 2 (S=1)
  # P(Y2,Y1|X,Z) = P(Y1|Y2,X,Z)P(Y2|X,Z)
  p_m1 <- ncol(Xmat_m1_s1)
  beta <- theta1[1:p_m1]
  alpha <- theta1[(p_m1 + 1):length(theta1)]
  alpha_ext <- c(-Inf, alpha, Inf)
  mu1 <- as.vector(Xmat_m1_s1 %*% beta)

  p_m2 <- ncol(Xmat_m2)
  obsll1 <- obs_loglik(theta2, p_m2, y2_s1, y1_s1, Xmat_m2, mu1, alpha_ext)

  # Subjects NOT selected in Phase 2 (S=0)
  # P(Y1|Z) = sum_v sum_l P(Y1|x_v, Z) * B_l(Z) * p_vl
  prob_y1s0_v <- compute_prob_matrix(theta1, y1_s0, Xmat_m1_s0, x_support, x_name)
  inner_sum <- prob_y1s0_v %*% p_vl
  prob0 <- pmax(1e-16, rowSums(inner_sum * B0))

  return(obsll1 + sum(log(prob0)))
}

# -- Profile Log-Likelihood ----
pl_theta12 <- function(theta_full, p_vl_init, p_vl_R1, p_covs1, y1_s0, y1_s1, y2_s1,
                       Xmat_m1_s0, Xmat_m1_s1, Xmat_m2, B0, x_support, x_name, max_iter = 100, tol = 1e-8) {

  t1 <- theta_full[1:p_covs1]
  t2 <- theta_full[(p_covs1 + 1):length(theta_full)]

  # Re-optimize p_vl for the current theta perturbation
  prob_y1s0_v <- compute_prob_matrix(t1, y1_s0, Xmat_m1_s0, x_support, x_name)
  p_vl_opt <- update_p_vl_cpp(prob_y1s0_v, B0, p_vl_init, p_vl_R1, max_iter, tol)

  return(calculate_full_loglik_theta12(t1, t2, p_vl_opt, y1_s0, y1_s1, y2_s1,
                                       Xmat_m1_s0, Xmat_m1_s1, Xmat_m2, B0, x_support, x_name))
}

# -- Estimate SE via profile likelihood ----
estimate_em_joint_se <- function(theta1_hat, theta2_hat, p_vl_hat, p_vl_R1,
                                 y1_s0, y1_s1, y2_s1, Xmat_m1_s0, Xmat_m1_s1, Xmat_m2,
                                 B0, x_support, x_name) {
  p_covs1 <- length(theta1_hat)
  # Define the function to differentiate
  obj_func <- function(t) {
    pl_theta12(theta = t, p_vl_init = p_vl_hat, p_vl_R1, p_covs1,
               y1_s0, y1_s1, y2_s1, Xmat_m1_s0, Xmat_m1_s1, Xmat_m2,
               B0, x_support, x_name)
  }

  theta_start <- c(theta1_hat, theta2_hat)
  # Calculate Hessian numerically
  hess <- numDeriv::hessian(func = obj_func, x = theta_start)

  # Estimate Covariance and SE
  vcov_mat <- try(solve(-hess), silent = TRUE)
  if (inherits(vcov_mat, "try-error")) {
    return(list(se = rep(NA, length(theta_start)), vcov = NULL))
  }
