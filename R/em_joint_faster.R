...
#' @export
em_joint_faster <- function(formula1, formula2, data, Bbasis, x_name,
                            theta1_init = NULL, theta2_init = NULL, se_calc = FALSE, max_iter = 500, tol = 1e-6) {
  ...
  for (iter in 1:max_iter) {
    ...
    y1star_moments_s1 <- compute_y1star_s1(theta1_curr, theta2_curr, y1_s1, y2_vec, Xmat_m1_s1, Xmat_m2)
    E_y1star_s1 <- y1star_moments_s1$E; Var_y1star_s1 <- y1star_moments_s1$Var
    ...
    # Update theta2
    theta2_curr <- update_theta2_joint(theta1_curr, theta2_curr, y1_s1, y2_vec, Xmat_m1_s1, Xmat_m2, E_y1star_s1, Var_y1star_s1)
    gamma_curr <- theta2_curr[1:p_covs2]

    # Update theta1
    theta1_curr <- update_theta_weighted(theta1_curr, w_iv, y1_s1, Xmat_m1_s1, y1_s0, Xmat_m1_s0, x_support, x_name)
  ...
  if (se_calc) {
    ...
    theta1_se <- se_res$se[1:p_covs1]
    theta2_se <- se_res$se[(p_covs1 + 1):length(se_res$se)]
    vcov_mat <- se_res$vcov
  }

  return(list(theta1 = theta1_curr, theta1_se = theta1_se,
              gamma = theta2_curr[1:p_covs2],
              sigma12 = theta2_curr[p_covs2 + 1],
              sigma22 = theta2_curr[p_covs2 + 2],
              theta2_se = theta2_se, iterations = iter))
}
